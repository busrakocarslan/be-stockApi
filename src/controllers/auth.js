...
module.exports = {
    // token varsa logindir. yoksa logout o yğzden logout da silinir token
        ...
        if ((username || email) && password) {
            ...
            if (user && user.password == passwordEncrypt(password)) {
                ...
                if (user.isActive) {
                    ...
                    // Simple TOKEN:
                    let tokenData = await Token.findOne({ userId: user._id })// daha önce token verilmiş mi kontrolü yapıyorum.
                    if (!tokenData) {
                        tokenData = await Token.create({
                            userId: user._id,
                            token: passwordEncrypt(user._id + Date.now())// standart bir veri çıkması için passwordEncrypt ile sarmalıyorum
                        })
                    }

                    // JWT:
                    //*ekstra data oluşturmadım direk user' ı adlığımdan json formatına çevirmem gerekiyor. çünkü jwt klasik obj kabul ediyordu ancak classdan oluşan ı toJSON() yapmak gerekiyordu.
    ...
    logout: async (req, res) => {
        ...
        */
        ...
        if (tokenKey) {
            ...
            } else { // JWT de logaout yok çünkü db ye kaydettiğimiz bir şey yok
